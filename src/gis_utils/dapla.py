...
def samle_filer(filer: list, **kwargs) -> gpd.GeoDataFrame:
    return pd.concat(
        (les_geopandas(fil, **kwargs) for fil in filer), axis=0, ignore_index=True
    )
...
def read_invalid(sti, layer=None, **kwargs):
    """Hvis fx en punktgeometri er kodet som LineString, klarer ikke geopandas å lese
    den. Leser da 1000 rader av gangen, gjentar for problemradene fram til suksess."""
    ...
    for i in np.arange(rader, step=1000):
        try:
            ...
            data = gpd.GeoDataFrame(
                pd.concat([data, df], axis=0, ignore_index=True),
                geometry="geometry",
                crs=crs,
            )
...
if __name__ == "__main__":
    x = read_invalid(
        r"C:\Users\ort\OneDrive - Statistisk sentralbyrå\data\ruteplanleggerSykkel_FGDB_20220601.gdb",
        layer="lenker_line",
        columns=[
            "startnode",
            "sluttnode",
    ...
    x.to_parquet(
        r"C:\Users\ort\OneDrive - Statistisk sentralbyrå\data\ruteplanleggerSykkel_2022.parquet"
    )
