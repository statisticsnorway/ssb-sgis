...
class Points:
    ...
    def make_temp_idx(self) -> None:
        ...
        self.gdf["temp_idx"] = np.arange(
            ...
            stop=self.temp_idx_start+len(self.gdf)
            )
        self.gdf["temp_idx"] = self.gdf["temp_idx"].astype(str)

        if self.id_col:
            self.id_dict = {temp_idx: idx for temp_idx, idx in zip(self.gdf.temp_idx, self.gdf[self.id_col])}
    ...
    def get_edges_and_costs(self, nodes: GeoDataFrame, rules: NetworkAnalysisRules, from_col: str, to_col: str):
