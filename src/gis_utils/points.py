...
class Points:
    ...
    def make_temp_idx(self) -> None:
        """Make a temporary id column that is not present in the node ids of the
        network. The original ids are stored in a dict and mapped to the results after
        the network analysis. This method has to be run after get_id_col, because this
        determines the id column differently for start- and endpoints.
        """

        self.points["temp_idx"] = np.arange(
            start=self.temp_idx_start, stop=self.temp_idx_start + len(self.points)
        )
        ...
        if self.id_col:
            self.id_dict = {
                temp_idx: idx
                for temp_idx, idx in zip(self.points.temp_idx, self.points[self.id_col])
            }
    ...
    def distance_to_nodes(
        ...
        """Creates a DataFrame with distances and indices of the 50 closest nodes for
        each point, then keeps only the rows with a distance less than the
        search_tolerance and the search_factor.
        """
...
class StartPoints(Points):
    ...
    def distance_to_nodes(
        ...
        """First runs the super method, which returns a DataFrame with distances and
         indices of the startpoints and the nodes,
        then an edgelist (of tuples) and a distance list is created."""
        df = super().distance_to_nodes(nodes, search_tolerance, search_factor)

        self.edges = [
            (temp_idx, node_id) for temp_idx, node_id in zip(df.temp_idx, df.node_id)
        ]
...
class EndPoints(Points):
    ...
    def distance_to_nodes(
        ...
        """Same as the method in the StartPoints class, but with opposite edge
        direction (node_id, point_id)."""
        df = super().distance_to_nodes(nodes, search_tolerance, search_factor)

        self.edges = [
            (node_id, temp_idx) for temp_idx, node_id in zip(df.temp_idx, df.node_id)
        ]
        self.dists = list(df.dist)
