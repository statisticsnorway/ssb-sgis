...
class NetworkAnalysis:
    ...
    def __init__(
        ...
        if not isinstance(rules, NetworkAnalysisRules):
            raise ValueError(f"'rules' should be of type NetworkAnalysisRules. Got {type(rules)}")

        if not isinstance(network, (Network, DirectedNetwork)):
            raise ValueError(f"'network' should of type DirectedNetwork or Network. Got {type(network)}")
    ...
    def get_edges_and_costs(self) -> tuple[list[tuple[str]], list[float]]:
        ...
        edges = [
            ...
            for source, target in zip(self.network.gdf["source"], self.network.gdf["target"])
            ]
    ...
    def add_missing_vertices(self):
        self.graph.add_vertices([idx for idx in self.startpoints.gdf["temp_idx"] if idx not in self.graph.vs["name"]])
        if self.endpoints is not None:
            self.graph.add_vertices([idx for idx in self.endpoints.gdf["temp_idx"] if idx not in self.graph.vs["name"]])
    ...
    def points_have_changed(self, points: GeoDataFrame, what: str):
        ...
        if not all(
            x in self.graph.vs["name"] for x in list(points.temp_idx.values)
            ):
            return True
