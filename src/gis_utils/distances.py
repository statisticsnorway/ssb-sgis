...
def get_k_nearest_neighbors(
    ...
    Args:
      ...
      min_dist (int): The minimum distance between the two points. Defaults to 0.0001
    ...
    df = df.assign(
        dist=dists, dist_min=lambda df: df.groupby(id_col1)["dist"].transform("min")
    )
...
def main():
    ...
    p = gpd.read_parquet(
        r"C:\Users\ort\OneDrive - Statistisk sentralbyrå\data\tilfeldige_adresser_10000.parquet"
    )
    p["temp_idx"] = p.index
    ...
    print(
        get_k_nearest_neighbors(
            p, r, id_cols=("temp_idx", "road_idx"), k=100
        )  # , max_dist=500)
    )

    print(p.sjoin_nearest(r, distance_col="dist"))
