...
def test_aggfuncs(gdf_fixture):
    ...
    copy = gs.buffdissexp(gdf_fixture, 100, by="txtkol", aggfunc=["sum", "mean"], copy=True)
    ...
    copy = gs.buffdissexp(gdf_fixture, 1000, by="txtkol", aggfunc=["sum", "mean"], copy=True)
...
def test_neighbors(gdf_fixture):
    naboer = gs.find_neighbors(gdf_fixture.iloc[[0]], possible_neighbors=gdf_fixture, id_col="numkol", within_distance = 100)
    naboer.sort()
    assert naboer==[1, 2], "feil i find_neighbors"
    naboer = gs.find_neighbors(gdf_fixture.iloc[[8]], possible_neighbors=gdf_fixture, id_col="numkol", within_distance = 100)
    naboer.sort()
