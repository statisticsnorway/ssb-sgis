...
def gdf_fixture() -> gpd.GeoDataFrame:
    ...
    xs = [10.7497196, 10.7484624, 10.7480624, 10.7384624, 10.7374624, 10.7324624,
          10.7284624]
    ys = [59.9281407, 59.9275268, 59.9272268, 59.9175268, 59.9165268, 59.9365268,
          59.9075268]
    ...
    gdf = gpd.GeoDataFrame({'geometry': gpd.GeoSeries(geoms)}, geometry="geometry",
                           crs=4326).to_crs(25833)

    gdf2 = to_gdf(geoms, crs=4326).to_crs(25833)

    assert gdf.equals(gdf2), "to_gdf does not give same results as manual gdf constructor"
    ...
    assert f'POINT ({x} {y})' == 'POINT (261106.48628 6649101.81219)', "wrong centerpoints. Have the testdata changed?"
