...
import sys
while "geopandasgreier_util" not in os.listdir():
    os.chdir("../")
sys.path.append(os.getcwd())

from geopandasgreier_util.buffer_dissolve_explode import buff, diss, buffdiss, dissexp, buffdissexp
from geopandasgreier_util.generelt import gdf_concat, to_gdf, fix_geoms, overlay_update
...
def test_fix(gdf):
    ...
    assert len(gdf)==11
    gdf = fiks_geometrier(gdf)
    assert len(gdf)==9
    ser = fiks_geometrier(gdf_concat([gdf, manglende, tomme]).geometry)
    assert len(ser)==9
